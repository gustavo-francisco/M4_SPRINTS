...
func _andar():
	if ScriptGlobal.direcaoAgua == 1:
		velocidade.x = 200
	
	elif ScriptGlobal.direcaoAgua == 2:
		velocidade.x = -200
...
func _pular():
	if ScriptGlobal.direcaoAgua == 3 and contagem_pulo < 2:
		velocidade.y = -400
		contagem_pulo += 1
...
func _on_gemaAzul_body_entered(body):
	if body == self:
		ScriptGlobal.gemaAzul1 = true

func _on_gemaAzul2_body_entered(body):
	if body == self:
		ScriptGlobal.gemaAzul2 = true

func _on_gemaAzul3_body_entered(body):
	if body == self:
		ScriptGlobal.gemaAzul3 = true
...
#caso o personagem entre no lago vermelho ele muda a cena para a cena de "morreu"
#e todas as variaveis de gema se resetam
func _on_Laguinho_Vermelho_body_entered(body):
	if body == self:
		get_tree().change_scene("res://cenas/morreu.tscn")
...
#caso o personagem entre na porta azul ele passa
func _on_portaAzul_body_entered(body):
	if body == self:
		ScriptGlobal.aguaPassou = true
	else:
		ScriptGlobal.aguaPassou = false
